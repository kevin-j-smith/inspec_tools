...
data = Nori.new(empty_tag_value: true).parse(File.read('audit.fvdl'))

data = JSON.parse(File.read('results_noko.json'))
...
def primaries(classid)
  ...
  matched_vulns.each do |vuln|
    ...
    traces.each do |trace|
      ...
      entries = entries.select { |x| !x['Node'].nil? }
...
File.write('inspec_new2.json', inpsec_json.to_json)
